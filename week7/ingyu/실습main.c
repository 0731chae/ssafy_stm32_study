...
volatile uint32_t last_edge_count = 0;
volatile uint8_t button_pressed = 0;

#define ONE_SECOND_COUNT 10000U
#define DEBOUNCE_COUNT     300U
...
int main(void)
{
  ...
  HAL_GPIO_WritePin(BLUE_LED_GPIO_Port,
                    BLUE_LED_Pin,
                    GPIO_PIN_RESET);
  /* USER CODE END 2 */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    if (GPIO_Pin == USER_BUTTON_Pin)
    {
        ...
        /* 약 30ms 디바운싱 */
        //if ((uint32_t)(now - last_edge_count) < 300)
        //{
            //return;
        //}
        ...
        else
        {
            ...
            if (button_pressed)
            {
                uint32_t elapsed = now - press_count;

                button_pressed = 0;

                if (elapsed >= ONE_SECOND_COUNT)
                {
                    /* 1초 이상 → RED */
                    HAL_GPIO_WritePin(RED_LED_GPIO_Port,
                                      RED_LED_Pin,
